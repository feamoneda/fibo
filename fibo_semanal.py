...
#data=['ANCBUSD','ANCUSDT','SLPBIDR','SLPETH','MIRBTC']
...
def fecha_hist(fecha):
    ...
    UTC_A = t - timedelta(days=40)
    UTC = t - timedelta(days=430)
...
for stock in df['symbol']:
    ...
    fecha_intervalo =  fecha_hist('21 March, 2022 04:00:00')
    #COEF-DELTA>MIE-VIE
    ...
    if (hist is not None):
            if( len(hist) > 3):
                ...
                #if( dif > 200):
                if( dif > 30 and promVolumen > 5000000):
                    start1 = fechaUTC_fibo(f[0])
                    #print(start1)
                    #print(f[1])
                    intervalo1 = Client.KLINE_INTERVAL_1WEEK
                    #hist1 = get_prices(stock, intervalo1, start1, f[1])
                    hist1 =  get_prices(stock, intervalo, fecha_intervalo[0], fecha_intervalo[1]) #get_prices(stock, intervalo1, start1, f[1])
                    iEMA = EMA(hist1)
                    iRSI = getRSI(hist1)
...
csv_url = os.path.join(dir_csv, '__FIBO_SEMANAL.csv')
