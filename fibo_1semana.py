...
#coinbase
#data=['ETHUSD','BTCUSD','AVAXUSD','SOLUSD','SHIBUSD','ATICUSD','MANAUSD','BTCEUR','ADAUSD','GALAUSD','USDTUSD','LINKUSD','ETHEUR','ACHUSD','ATOMUSD','ALGOUSD','ICPUSD','ETHUSDT','LTCUSD','BTCUSDT','XLMUSD','BTCGBP','ETHBTC','DOTUSD','DOGEUSD','RLYUSD','BTCUSDC','ETHGBP','LUNAUSD','ALAUSDT','CROUSD','ETHUSDC','USDTEUR','CRVUSD','FILUSD','GRTUSD','LRCUSD','SOLEUR','ASMYUSD','SOLUSDT','MIRUSD','ZECUSD','AMPUSD','UNIUSD','VAXUSDT','XTZUSD','AAVEUSD','MDTUSD','ADAEUR','AVAXEUR','ADAEUR','USTUSD','BCHUSD','AXSUSD','ZRXUSD','SOLBTC']
#data = ['ALGOUSDT', 'ALGOBTC']
#data=['NBTBIDR']
#data = ['BLZUSDT','LRCBTC','OGNUSDT','MITHUSDT','REQBTC','SLPUSDT','BRDBTC']
#data = ['ETCBTC']
#data=['ICXUSDT','STXUSDT','MBLBNB','MBLUSDT','LAZIOTRY','LAZIOUSDT','PORTOUSDT','PORTOTRY','SANTOSUSDT','SANTOSTRY','ANCUSDT','ALPINETRY','ALPINEUSDT']
#data = ['RAREBTC','HIGHBTC','JASMYBTC','JASMYBTC','AUCTIONUSDT','RAREUSDT','APEUSDT','APEBTC']
#FAVS
#data = ['ALGOUSDT','BTCUSDT','ETHUSDT','AVAXUSDT','SOLUSDT','MATICUSDT','ADAUSDT','ETHEUR','GALAUSDT','MANAUSDT','WINUSDT','WINUSDC','XRPDOWNUSDT','SLPUSDT','SHIBUSDT','SHIBBUSD','SHIBEUR','SHIBBRL','SHIBTRY','XECBUSD','XECUSDT','SHIBAUD','SHIBDOGE','BTTCUSDT','BTTCUSDC','BTTCTRY']
#data = ['SLPUSDT']
data = ['BRDBTC']
#data = ['BTCUSDT']
#data = ['ADADOWNUSDT','LINKDOWNUSDT','DOTDOWNUSDT','API3BTC','TORNBUSD','TORNUSDT','BETABTC','BETABNB','API3USDT','API3BUSD','INJBTC','ONGBTC','INJBNB','BETAUSDT','BETABUSD']
...
def fechaUTC_rev(fecha):
    ...
    UTC = t - timedelta(days=10)
    UTC1 = t #UTC #- timedelta(days=2)
    #UTC2_3 = t - timedelta(hours=3)
    #if (b == 1):
    #    print('RSI/',str(UTC1_3), str(UTC2_3))
...
def EMA(dataframe):
    if (dataframe is not None):
        ...
        if(len(dataframe) > 3):
            ...
            EMA = ta.EMA(dataframe.Close.values, 14)
            EMA = round(EMA[-1],2)
            #print(signal[-1])
            print('EMA:',EMA)
...
def CCI(dataframe):
    if( dataframe is not None and dataframe.empty is not True and len(dataframe) > 6):
        ...
        dataframe.Close = dataframe.Close.astype(np.float64)
        dataframe.Volumen = dataframe.Volumen.astype(np.float64)
        CCI = ta.CCI(dataframe.High, dataframe.Low, dataframe.Close, 14)
...
def crearImagen(senales):
    ...
    img_url = os.path.join(dir, 'img' , 'background23hs.jpg')
...
def fecha_hist(fecha):
    t = datetime.strptime(fecha, '%d %B, %Y %H:%M:%S')
    t = t + timedelta(hours=3)
    UTC_A = t - timedelta(days=5)
    UTC = t - timedelta(days=20)
    print(str(UTC_A), str(t), str(UTC))
    return str(UTC_A), str(t), str(UTC)
...
e=[]
hoy = date.today()
fecha_hoy = hoy.strftime("%d %B, %Y 21:00:00")
signals =  []
...
for stock in df['symbol']:
    ...
    fecha_intervalo =  fecha_hist('17 March, 2022 19:00:00')
    #COEF-DELTA>MIE-VIE
    f = fechaUTC_format(fecha_hoy)
    #hist = get_prices(stock, intervalo, f[0], f[1])
    hist = get_prices(stock, intervalo, fecha_intervalo[0], fecha_intervalo[1])
    ...
    if (hist is not None):
            if( len(hist) > 3):
                ...
                difR0 = ( ( fiboind['R0'] / fiboind['S0'] ) * 100 ) - 100
                difR1 = ( ( fiboind['R1'] / fiboind['S0'] ) * 100 ) - 100
                #start_rev = fechaUTC_rev(f[0])
                print('---------------------------')
                #hist_rev = get_prices(stock, intervalo, '2022-01-27 00:00:00', '2022-02-07 00:00:00')
                print(stock, int(difR0))
                #print(hist_rev)
                ...
                if(difR0 > 2):
                    ...
                    print("INTERVALO HIST1", fecha_intervalo[0], fecha_intervalo[1])
                    #print(f[1])
                    intervalo1 = Client.KLINE_INTERVAL_1DAY
                    #hist1 = get_prices(stock, intervalo1, start1, f[1])
                    hist1 =  get_prices(stock, intervalo, fecha_intervalo[2], fecha_intervalo[0]) #get_prices(stock, intervalo1, start1, f[1])
                    print("hist1",hist1)
                    if( hist1 is not None):
                        iEMA = EMA(hist1)
                        iRSI = getRSI(hist1)
                        iMA = ta.MA(hist1.Close, timeperiod=12, matype=0)
                        ...
                        if( iRSI is not None and iEMA is not None and iMA is not None ): #and r is not None):
                            ...
                            #difr0 = ( fiboind['R1'] / fiboind['R0'] * 100 ) - 100
                            mom = MOM(hist1.Close)
                            roc = ROC(hist1.Close)
                            cmo = CMO(hist1)
                            stoch = iSTOCH(hist1)
                            ##EVALUAR 2HR PREVIAS >>> RSI 50~+60 / STOCH RSI 1-30~60 2-30~40 rsi1==rsi2 REVERSAL / CCI
                            iCCI = CCI(hist1) ##PROBAR CON REGISTROS DE 20 DIAS
                            coef = get_coef(hist1)
                            c = coef[1][0]
                            #print('t',f[0])
                            ...
                            if (mom is not None and cmo is not None and roc is not None and iCCI is not None):
                                if( r['ENG_Bool']): # == True and roc[0] > 15):
                                    cmo_count += 1
                                ...
                                #    tweet.append('#criptomonedas #binancearg #binance')
                                    ...
                                    senalesdata.append( [ str(stock), str.format('{0:.0f}',difR0), str.format('{0:.8f}',fiboind['R0']), str.format('{0:.8f}',fiboind['R1']) ] )
                                    print( str(stock),'\t',str.format('{0:.1f}',difR0), str(iMA), str(coef[1][0]), str.format('{0:.2f}', delta), str.format('{0:.8f}',fiboind['Actual']), str.format('{0:.8f}',fiboind['S1']),str.format('{0:.8f}',fiboind['R0']),str.format('{0:.8f}',fiboind['R1']),str.format('{0:.8f}',fiboind['R2']),'Up',str(r['Aroon_Up']),'Down',str(r['Aroon_Down']))
                                    e.append(
                                                {
                                                'Ticker' : str(stock),
                                                ...
                                                'DifS0R0' : str.format('{0:.0f}',difR0),
                                                'DifS0R1' : str.format('{0:.0f}',difR1),
                                                'EMA' : str(iEMA),
                                                'MA' : str(iMA),
                                                'CCI-20' : str(iCCI[1]),
                                                'CCI-21' : str(iCCI[0]),
...
csv_url = os.path.join(dir_csv, '__FIBO3MESES_.csv')
