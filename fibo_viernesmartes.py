...
data = ['ANTBTC','NEARBUSD','NEARUSDT','SUSHIUSDT','HNTBUSD','HNTUSDT','AVAXBUSD','AVAXUSDT','MATICGBP','MATICEUR','AVAXBRL','MATICAUD','AVAXEUR','MATICBUSD','MATICUSDT','MATICRUB','MATICBRL','MATICBIDR','IOTXETH','MATICBNB']
    ...
    if( dataframe is not None and dataframe.empty is not True):
        ...
        if(dataframe is not None):
            ...
            if(dataframe is not None and dataframe.empty is not True):
               ...
               print(str(fechaFormat_fiboind(f3)) , 'Max:', hactual, '<', str(float(round(a,8))), (dataframe.Close[0] < float(round(a,8))))
               aalto = hactual
            else:
               aalto = 0
        ...
        cr23 = (r2*Decimal(1.03))
        aexp = (a*Decimal(1.04))
        cgz = difOkrs(cr23, 'r', bajo, alto)
...
def get_fibo(dataframe):
    if( dataframe is not None and dataframe.empty is not True):
        ...
        #print('FIBOINDICES', 'Actual', str(actual), 'S0', str(s0),'R0', str(r0))
        return fibo
...
def fechaUTC_format(fecha):
    #print(a)
    #("21 December, 2021 03:00:00")
    #orig 2021-12-17 00:00:00 2021-12-20 21:00:00
    t = datetime.strptime(fecha, '%d %B, %Y %H:%M:%S')
    UTC1 = t - timedelta(days=3, hours=21)
...
def fecha_comp(fecha):
    #print(v)
    t = datetime.strptime(fecha, '%Y-%m-%d %H:%M:%S')
    UTC1 = t + timedelta(days=4)
    UTC2 = UTC1 + timedelta(days=2, hours=20)
    UTC1_3 = UTC1 - timedelta(hours=3)
    UTC2_3 = UTC2 - timedelta(hours=3)
    if (v == 1):
...
def reversal(dataframe):
    if( dataframe is not None and dataframe.empty is not True and len(dataframe) >= 6):
        ...
        #print('Up', aroon[0])
        #print('Down', aroon[1])
        result = { 'Aroon_Up' : aroon_up, 'Aroon_Down': aroon_down, 'ENG_Bool' : b, 'MBOZU_Bool' : mb, 'ENG_17': eng.iloc[0], 'ENG_18': eng.iloc[1],'ENG_19': eng.iloc[2],'ENG_20': eng.iloc[3],'ENG_21': eng.iloc[4],'MBOZU_17': mbozu_close.iloc[0], 'MBOZU_18': mbozu_close.iloc[1],'MBOZU_19': mbozu_close.iloc[2],'MBOZU_20': mbozu_close.iloc[3],'MBOZU_21': mbozu_close.iloc[4]}
        return result
...
def CMO(dataframe):
    if( dataframe is not None and dataframe.empty is not True and len(dataframe) > 3):
        ...
        dataframe.Low = dataframe.Low.astype(np.float64)
        dataframe.Close = dataframe.Close.astype(np.float64)
        CMO = ta.CMO(dataframe.Close, timeperiod=14)
        #print('CMO',str(CMO.iloc[-1],str(CMO.iloc[-2]),' >> ', str(CMO.iloc[-1]-CMO.iloc[-2]) ))
        if (CMO is not None):
...
def MOM(dataframe):
    if( dataframe is not None and dataframe.empty is not True and len(dataframe) > 3):
        mom = ta.MOM(dataframe, timeperiod=14)
        if (mom is not None):
...
def ROC(dataframe):
    if( dataframe is not None and dataframe.empty is not True and len(dataframe) > 3):
        roc = ta.ROC(dataframe, timeperiod=14)
        if (roc is not None):
...
def crearImagen(senales):
    ...
    img = Image.open(r"/home/torrescolaneri/background1.jpg").convert('RGB')
    draw = ImageDraw.Draw(img)

    font = ImageFont.truetype("/home/torrescolaneri/fonts/OpenSans_Condensed-SemiBold.ttf", 48)
    font0 = ImageFont.truetype("/home/torrescolaneri/fonts/OpenSans_Condensed-Bold.ttf", 50)
    font1 = ImageFont.truetype("/home/torrescolaneri/fonts/OpenSans_Condensed-SemiBold.ttf", 36)
    font2 = ImageFont.truetype("/home/torrescolaneri/fonts/OpenSans-LightItalic.ttf", 70)
    ...
    #str_fname_url = urljoin('C/Users/Ana/Downloads',str_fname)
    str_fname_url = "/home/torrescolaneri/"
    str_file_path = str(str_fname_url+str_fname)
    mediasrotas.append(str_file_path)
    # saving the image
    img.save(str_fname_url+str_fname)

def IMA(dataframe):
    if (dataframe is not None and dataframe.empty is not True):
        i = ta.MA(dataframe.Close, timeperiod=14, matype=0)
        if (i.empty is True):
            i = 0
        else:
            i = round(i.iloc[-1],4)
...
edf = pd.DataFrame(columns = ['Ticker','Coef','Delta','EMA','MA','CCI-1','CCI-2','AROON_UP','AROON_DOWN','ROC','MOM','CMO','STOCH_K','STOCH_D','RSI','Vol','Max','Actual','5%','Max','Min','DifR2','Dif_High','P','S0','S1','S2','S3','R0','R1','R2','R3'])

e=[]
hoy = date.today()
fecha_hoy = hoy.strftime("%d %B, %Y 03:00:00")
...
for stock in df['symbol']:
    ...
    #COEF-DELTA>MIE-VIE
    f = fechaUTC_format("21 December, 2021 03:00:00")
            ...
            if( len(hist) > 3):
                ...
                if (start_rev is not None and delta is not None and r is not None and promVolumen is not None) :
                    ...
                    if( iRSI is not None and iEMA is not None and iMA is not None ): #and r is not None):
                        ...
                        if (dif is not None and str(iMA) is not None and fibocom is not None and str(coef[1][0]) is not None and delta is not None and str.format('{0:.8f}',fiboind['Actual']) is not None and str.format('{0:.8f}',fiboind['S0']) is not None and str.format('{0:.8f}',fiboind['R0']) is not None and str.format('{0:.8f}',fiboind['R1']) is not None and str.format('{0:.8f}',fiboind['R2']) is not None and str.format('{0:.8f}',r['Aroon_Up']) is not None and str.format('{0:.8f}',r['Aroon_Down']) is not None):
                            print( str(stock),'\t','%',str.format('{0:.1f}',dif),'--',  'R0_COMP', str(fibocom[21]),'R1_COMP', str(fibocom[3]),'--', str(iMA), str(coef[1][0]), str.format('{0:.2f}', delta), str.format('{0:.8f}',fiboind['Actual']), 'S0:',str.format('{0:.8f}',fiboind['S0']),'R0:',str.format('{0:.8f}',fiboind['R0']),'R1:',str.format('{0:.8f}',fiboind['R1']),str.format('{0:.8f}',fiboind['R2']),'Up',str(r['Aroon_Up']),'Down',str(r['Aroon_Down']))

                            if (c > 100000):
                                signal = ':ticket: ' + stock + '\t' + ' Buy: ' + str.format('{0:.5f}',fiboind['S0']) +' Target1: ' + str.format('{0:.8f}',fiboind['R0']) + ' Target2: '+  str.format('{0:.8f}',fiboind['R1']) + ' Estim. %'+ str.format('{0:.0f}',dif)
                                ...
                                tweet.append('🟩 Buy: ~' + str.format('{0:.5f}',fiboind['S0']) )
                            ...
                            e.append(
                                        ...
                                        'ENG_20': str(r['ENG_20']),
                                        'ENG_21': str(r['ENG_21']),
                                        'MBOZU_17': str(r['MBOZU_17']),
                                        'MBOZU_18': str(r['MBOZU_18']),
                                        'MBOZU_19': str(r['MBOZU_19']),
                                        'MBOZU_20': str(r['MBOZU_20']),
                                        'MBOZU_21': str(r['MBOZU_21']),
...
#'R0_COMP' : str(fibocom[21]),
#'R1_COMP' : str(fibocom[3]),
#dR1 = round((len(acr1)/len(edf)*100),2)
...
edf.to_csv('/home/torrescolaneri/__FIBO_VIERNES_MARTES_' +fechaFN(f[0])+'.csv', index=False)
fs_filepath = ('/home/torrescolaneri/__FIBO_VIERNES_MARTES_' + fechaFN(f[0]) + '.csv')
fs_filepath_name = ('__FIBO_VIERNES_MARTES_' + fechaFN(f[0]) + '.csv')
...
my_file = {
  'file' : ('/home/torrescolaneri/__FIBO_VIERNES_MARTES_'+fechaFN(f[0])+'.csv', 'csv')
}
