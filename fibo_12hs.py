...
def fechaUTC_fibo(fecha):
    #print(b)
    t = datetime.strptime(fecha, '%Y-%m-%d %H:%M:%S')
    UTC = t #- timedelta(days=7, hours=3)
    UTC1_3 = UTC + timedelta(days=7, hours=3)
...
def fecha_comp(fecha):
    #print(v)
    t = datetime.strptime(fecha, '%Y-%m-%d %H:%M:%S')
    UTC1 = t + timedelta(days=6)
    UTC2 = UTC1 + timedelta(days=5)
    UTC1_3 = UTC1 - timedelta(hours=3)
    UTC2_3 = UTC2 - timedelta(hours=3)
    if (v == 1):
...
e=[]
hoy = date.today()
fecha_hoy = hoy.strftime("%d %B, %Y 08:00:00")
