...
#coinbase
#data = ['ETHEUR','QNTUSD','ETHBTC','XLMUSD','DOGEUSD','XTZUSD','DOTUSD','BTCGBP','JASMYUSD','BTCUSDT','IMXUSD','ICPUSD','BCHUSD','GALAUSD','ETHGBP','ETCUSD','REQUSD','CROUSDT','AMPUSD','SPELLUSD','GRTUSD','CRVUSD','QSPUSD','ACHUSD','ENJUSD','SHPINGUSD','UNIUSD','ANKRUSD','XYOUSD','BATUSD','ADAEUR','AAVEUSD','SOLEUR','WLUNAUSD','BTCUSDC']
...
for stock in df['symbol']:
    ...
    hist = get_prices(stock, intervalo, '2021-10-16 00:00:00', '2022-2-7 00:00:00')
    # print('for',type(hist))
    if (hist is not None):
            if( len(hist) > 3):
                delta = getDelta(hist)
                ...
                if( dif > 200):
                    ...
                    hist1 =  get_prices(stock, intervalo, '2021-10-16 00:00:00', '2022-2-7 00:00:00') #get_prices(stock, intervalo1, start1, f[1])
                    iEMA = EMA(hist1)
                    iRSI = getRSI(hist1)
